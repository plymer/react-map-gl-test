...
export interface IAnimationContext {
  ...
  setCurrentFrame: React.Dispatch<
    React.SetStateAction<IAnimationContext["currentFrame"]>
  >;
  frameCount: number; // how many frames are in the animation
  setFrameCount: React.Dispatch<
    React.SetStateAction<IAnimationContext["frameCount"]>
  >;
  frameRate: number;
  setFrameRate: React.Dispatch<
    React.SetStateAction<IAnimationContext["frameRate"]>
  >;
  startTime: number;
  setStartTime: React.Dispatch<
    React.SetStateAction<IAnimationContext["startTime"]>
  >;
  endTime: number;
  setEndTime: React.Dispatch<
    React.SetStateAction<IAnimationContext["endTime"]>
  >;
  timeStep: number;
  setTimeStep: React.Dispatch<
    React.SetStateAction<IAnimationContext["timeStep"]>
  >;
}
...
export const AnimationContextProvider = ({
  ...
  const [animationState, setAnimationState] =
    useState<IAnimationContext["animationState"]>(false);
  ...
  const [timeStep, setTimeStep] =
    useState<IAnimationContext["timeStep"]>(60000);
